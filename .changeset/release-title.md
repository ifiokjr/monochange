...
**`release_title`** — plain text title for provider releases (GitHub, GitLab, Gitea). **`changelog_version_title`** — markdown-capable title for changelog version headings.
...
> **Breaking change** — changelog version headings now include the release date by default (e.g. `## 1.2.3 (2026-04-06)` instead of `## 1.2.3`). Namespaced packages also include the package/group id prefix. To restore the previous format, set `changelog_version_title = "{{ version }}"` in `[defaults]`.
