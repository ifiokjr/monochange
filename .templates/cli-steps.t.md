...
- **config-driven workflow commands**: every `[cli.<command>]` table in `monochange.toml` becomes `mc <command>`. `mc init` writes starter workflow commands such as `discover`, `change`, `release`, `publish`, and `affected`; edit those tables when you want a named workflow that chains steps, adds custom inputs, or runs `Command` steps.
