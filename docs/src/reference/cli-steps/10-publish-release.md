...
It does not publish package artifacts to registries. Package publishing lives in the built-in top-level `mc publish`, `mc publish-readiness`, and `mc placeholder-publish` commands.
...
Use `mc publish` instead when you want monochange to run `cargo publish`, `pnpm publish`, `dart pub publish`, `flutter pub publish`, or `deno publish` style package-registry commands. Run `mc publish-readiness --from HEAD --output <path>` first only when you want a reviewable preflight report.
