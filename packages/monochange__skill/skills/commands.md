...
- The root `monochange.toml` for configured `[cli.*]` workflow commands such as `release`, `change`, and `publish-check` in this repository.
...
| `mc step:release-record`           | `ReleaseRecord`         | Inspect a release record from a tag or commit.                          |
| `mc step:publish-readiness`        | `PublishReadiness`      | Check package registry publishing readiness.                            |
| `mc step:publish-packages`         | `PublishPackages`       | Publish package versions from a publish plan.                           |
| `mc step:tag-release`              | `TagRelease`            | Create release tags from a release record.                              |
