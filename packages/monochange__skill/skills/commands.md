...
| Publish package artifacts       | `mc publish --readiness <path> --output <path>` | Built-in package publishing is configured and readiness passed                   |
...
- `mc publish-readiness --from HEAD --output <path>`, optional `mc publish-bootstrap --from HEAD --output <path>`, `mc publish-plan --readiness <path>`, and `mc publish --readiness <path> --output <path>` to plan/bootstrap/publish package artifacts; rerun with `--resume <path>` after a partial publish failure
