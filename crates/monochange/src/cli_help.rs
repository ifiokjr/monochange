...
fn builtin_command_helps() -> Vec<CommandHelp> {
	vec![
		CommandHelp {
			...
			description:
				"Scans the workspace for supported package manifests (Cargo.toml, package.json, \
			...
			options: &[
				...
				("--provider", "<PROVIDER>", "Source-control provider (github, gitlab, gitea)"),
			],
			examples: &[
				("Initialize a fresh workspace:", "mc init"),
				("Overwrite existing config:", "mc init --force"),
				("Initialize with GitHub integration:", "mc init --provider github"),
				("Initialize with GitLab integration:", "mc init --provider gitlab"),
				("Initialize with Gitea integration:", "mc init --provider gitea"),
			],
			tips: &[
		...
		CommandHelp {
			...
			description:
				"Compares the built-in default CLI commands against what is defined in \
			...
			options: &[],
			examples: &[
				("Add any missing default commands:", "mc populate"),
			],
			tips: &[
		...
		CommandHelp {
			...
			description:
				"Installs monochange-specific skills into the current project using the \
			...
			options: &[
				("(forwarded)", "", "All args are forwarded to `skills add`"),
			],
			examples: &[
				...
				("Install specific skill with copy:", "mc skill --skill monochange --copy -y"),
				("Install for multiple agents:", "mc skill -a claude-code -a codex"),
			],
			tips: &[
		...
		CommandHelp {
			...
			description:
				"Generates AI agent configuration files (markdown instructions, MCP config, \
			...
			examples: &[
				...
				("Generate without MCP config:", "mc subagents vscode copilot --no-mcp"),
			],
			tips: &[
		...
		CommandHelp {
			...
			description:
				"Runs package-scoped semantic analysis comparing a package's public API across \
			...
			examples: &[
				...
				("Against a specific release tag:", "mc analyze --package core --release-ref core/v1.2.3"),
				("Custom main and head refs:", "mc analyze --package core --main-ref main --head-ref HEAD"),
			],
			tips: &[
		...
		CommandHelp {
			...
			description:
				"Creates a structured changeset markdown file in .changeset/ that describes \
			...
			options: &[
				...
				("--package", "<PACKAGE>", "Package or group to include (repeatable)"),
				("--bump", "<BUMP>", "none, patch, minor, major (default: patch)"),
				("--version", "<VERSION>", "Pin an explicit version for this release"),
				("--reason", "<REASON>", "Short release-note summary"),
				("--type", "<TYPE>", "Release-note type (feat, fix, security, etc.)"),
				("--caused-by", "<CAUSED_BY>", "Ids that caused this dependent change (repeatable)"),
				("--details", "<DETAILS>", "Multi-line release-note details"),
				("--output", "<PATH>", "Write the change file to a specific path"),
			],
			examples: &[
				("Quick patch for a single package:", r#"mc change --package core --bump patch --reason "fix null pointer""#),
				("Minor feature with output path:", r#"mc change --package api --bump minor --reason "add pagination" --output .changeset/api-pagination.md"#),
				("Group-level change:", r#"mc change --package sdk --bump minor --reason "coordinated release""#),
				("Dependency-only follow-up:", r#"mc change --package utils --bump patch --caused-by core --reason "bump for core compat""#),
				("Interactive mode:", "mc change --interactive"),
				("Explicit version pin:", r#"mc change --package core --bump major --version 2.0.0 --reason "promote to stable""#),
			],
			tips: &[
		...
		CommandHelp {
			...
			description:
				"Reads all changeset files in .changeset/, plans version bumps and changelog \
			...
			options: &[
				...
				("--format", "<FORMAT>", "markdown, text, json (default: markdown)"),
				("--prepared-release", "<PATH>", "Read/write prepared release artifact path"),
			],
			examples: &[
				("Dry-run preview in text format:", "mc release --dry-run --format text"),
				("Dry-run with JSON for scripting:", "mc release --dry-run --format json"),
		...
		CommandHelp {
			...
			description:
				"Computes the same planned versions used by mc release but renders them without \
		...
		CommandHelp {
			...
			description:
				"Creates a git commit that embeds a durable monochange release record in the \
		...
		CommandHelp {
			...
			description:
				"Opens (or updates an existing) pull request on the configured source host \
		...
		CommandHelp {
			...
			description:
				"CI-oriented command that evaluates whether changed paths are adequately covered \
			...
			options: &[
				("--changed-paths", "<PATHS>", "File paths changed in the PR (repeatable)"),
				("--since", "<REF>", "Git ref to diff against (e.g. origin/main)"),
				("--verify", "", "Verify changeset coverage for affected packages"),
				("--label", "<LABELS>", "PR labels that may skip verification (repeatable)"),
				("--format", "<FORMAT>", "text, json (default: text)"),
			],
			examples: &[
				("Check specific changed paths:", "mc affected --changed-paths crates/core/src/lib.rs --format json"),
				("Compare against a branch:", "mc affected --since origin/main --verify"),
				("In CI with labels:", "mc affected --since origin/main --label skip-changeset"),
			],
			tips: &[
		...
		CommandHelp {
			...
			description:
				"Dumps detailed structured information about changeset files including: \
			...
			options: &[
				("--changeset", "<PATH>", "Specific changeset file(s) to diagnose (repeatable)"),
				("--format", "<FORMAT>", "text, json (default: text)"),
			],
			examples: &[
				("Diagnose all changesets:", "mc diagnostics"),
				("JSON output:", "mc diagnostics --format json"),
				("Specific changeset:", "mc diagnostics --changeset .changeset/feature.md"),
			],
			tips: &[
		...
		CommandHelp {
			...
			description:
				"Finds the release record from history and moves the full release tag set to \
			...
			options: &[
				("--from", "<REF>", "Tag or commit-ish locating the release (required)"),
				("--target", "<REF>", "Target commit (default: HEAD)"),
				("--force", "", "Allow retarget to non-descendant commits"),
				("--sync-provider", "=BOOL", "Sync hosted release (default: true)"),
				("--dry-run", "", "Preview without modifying tags"),
				("--format", "<FORMAT>", "text, json (default: text)"),
			],
			examples: &[
				("Dry-run repair:", "mc repair-release --from v1.2.3 --dry-run"),
				("Repair to HEAD:", "mc repair-release --from v1.2.3 --target HEAD"),
				("Force retarget:", "mc repair-release --from v1.2.3 --target HEAD --force"),
				("Skip provider sync:", "mc repair-release --from v1.2.3 --sync-provider=false"),
			],
			tips: &[
		...
		CommandHelp {
			...
			description:
				"Reads the monochange release record embedded in a commit's body and creates \
			...
			examples: &[
				...
				("Create without pushing:", "mc tag-release --from HEAD --push=false"),
				("JSON output:", "mc tag-release --from HEAD --dry-run --format json"),
			],
			tips: &[
		...
		CommandHelp {
			...
			description:
				"Resolves the supplied ref to a commit, then walks first-parent ancestry until \
			...
			examples: &[
				...
				("JSON output:", "mc release-record --from v1.2.3 --format json"),
			],
			tips: &[
		...
		CommandHelp {
			...
			description:
				"Validates monochange.toml, changeset files, and runs ecosystem-specific \
			...
			options: &[
				...
				("-e, --ecosystem", "<ECOSYSTEMS>", "Limit to specific ecosystem suites (comma-sep)"),
				("--only", "<RULES>", "Run only specific lint rule ids (comma-sep)"),
				("--format", "<FORMAT>", "text, json (default: text)"),
			],
			examples: &[
				...
				("Specific rule:", "mc check --only cargo/sorted-dependencies"),
			],
			tips: &[
		...
		CommandHelp {
			...
			description:
				"Subcommand group for listing, explaining, and creating lint rules that \
			...
			examples: &[
				...
				("Explain a rule:", "mc lint explain cargo/sorted-dependencies"),
				("Create a new rule:", "mc lint new cargo/no-path-dependencies"),
				("Create npm rule:", "mc lint new npm/require-package-manager"),
			],
			tips: &[
		...
		CommandHelp {
			...
			description:
				"Starts a Model Context Protocol (MCP) server that exposes monochange \
		...
		CommandHelp {
			...
			description:
				"Validates the monochange.toml configuration, package manifests, version \
			...
			options: &[],
			examples: &[
				("Validate the workspace:", "mc validate"),
			],
			tips: &[
		...
		CommandHelp {
			...
			description:
				"Scans the workspace for packages across Cargo, npm/pnpm/Bun, Deno, and \
		...
		CommandHelp {
			...
			description:
				"Packages that have never been published to their target registry (crates.io, \
			...
			options: &[
				("--format", "<FORMAT>", "text, markdown, json (default: text)"),
				("--package", "<PACKAGE>", "Restrict to specific package ids (repeatable)"),
				("--dry-run", "", "Preview without publishing"),
			],
			examples: &[
				("Dry-run all:", "mc placeholder-publish --dry-run"),
				("Specific package:", "mc placeholder-publish --package core --dry-run"),
				("JSON output:", "mc placeholder-publish --dry-run --format json"),
			],
			tips: &[
		...
		CommandHelp {
			...
			description:
				"Publishes package versions to their target registries using the prepared \
			...
			examples: &[
				...
				("JSON format:", "mc publish-packages --dry-run --format json"),
			],
			tips: &[
...
pub fn render_overview_help(bin_name: &str) -> String {
	...
	out.push_str(&format!(
		"  {}\n",
		paint("Use `mc help <command>` for detailed command help.", accent()),
	));
...
fn render_unknown_command_help(bin_name: &str, command_name: &str, helps: &[CommandHelp]) -> String {
	let mut out = String::new();
	out.push_str(&format!(
		"{} Unknown command `{}`\n\n",
