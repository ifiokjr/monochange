...
pub fn run_from_env(bin_name: &'static str) -> MonochangeResult<()> {
	...
	if !quiet && !output.is_empty() {
		let format = detect_output_format_from_env_args();
		if format == OutputFormat::Markdown && std::io::stdout().is_terminal() {
...
fn detect_output_format_from_env_args() -> OutputFormat {
