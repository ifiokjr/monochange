...
impl LintSuite for ChangesetLintSuite {
	...
	fn collect_targets(
		...
		for entry_result in fs::read_dir(&changeset_dir)
			.map_err(|error| MonochangeError::Io(format!("failed to read changeset directory: {error}")))? {
			let entry = entry_result
				.map_err(|error| MonochangeError::Io(format!("failed to read changeset directory entry: {error}")))?;
			let path = entry.path();
			...
			let contents = fs::read_to_string(&path)
				.map_err(|error| MonochangeError::Io(format!("failed to read changeset file: {error}")))?;
		...
		for change_type in change_types {
			if markdown_has_heading(&file.body, change_type) {
				results.push(LintResult::new(
					...
					LintLocation::new(&ctx.manifest_path, 1, 1),
					format!(
						"changeset type `{change_type}` must not also be used as a heading"
					),
					severity,
				));
			}
	...
	fn new(bump: BumpSeverity) -> Self {
		Self {
			rule: LintRule::new(
				...
				format!(
					"Requires changesets with bump `{bump}` to satisfy scope rules"
				),
				LintCategory::Correctness,
				LintMaturity::Stable,
				false,
	...
	fn run(&self, ctx: &LintContext<'_>, config: &LintRuleConfig) -> Vec<LintResult> {
		...
		for change in &file.changes {
			...
			if let Some(min_chars) = min_body_chars {
				if file.body.trim().chars().count() < min_chars {
					results.push(LintResult::new(
						self.rule.id.clone(),
						LintLocation::new(&ctx.manifest_path, 1, 1),
						format!(
							"changeset body must be at least {min_chars} characters"
						),
						severity,
					));
				}
			...
			if let Some(max_chars) = max_body_chars {
				if file.body.trim().chars().count() > max_chars {
					results.push(LintResult::new(
						self.rule.id.clone(),
						LintLocation::new(&ctx.manifest_path, 1, 1),
						format!(
							"changeset body must be at most {max_chars} characters"
						),
						severity,
					));
				}
