...
mod tests {
	...
	fn parser_backed_symbol_collection_handles_named_default_exports() {
		...
		let default_interface = PackageSnapshotFile {
			path: PathBuf::from("src/index.ts"),
			contents: "export default interface GreetingContract {}\n".to_string(),
		};

		let function_symbols = collect_public_symbols(&default_function);
		...
		assert!(interface_symbols.iter().any(|symbol| {
			symbol.item_kind == "interface" && symbol.item_path == "GreetingContract"
		}));
	}
